...
function TotalEdgeWeights(network::NetWork, node::Integer)
...
function TotalEdgeWeightsPerNode(network::NetWork)
  ...
  map(i -> total_edge_weights_per_node[i] = TotalEdgeWeights(network, i), 1:(network.nodes_number))
  return total_edge_weights_per_node
end

@inline function reduce_weights!(
  m::Int,
  i::Int,
  j::Base.RefValue,
  network::NetWork,
...
@inline function reduce_weights2!(
  k::Int,
  edges_number::Int,
  ...
  reduced_network_neighbor1[edges_number + k] = reduced_network_neighbor2[k]
...
@inline function ReducedNet(network::NetWork, clustering::MClustering)
  edges_number = 0
  ...
  @inbounds @fastmath @simd for i in 1:(clustering.clusters_number)
    ...
    @inbounds @fastmath @simd for k in 1:length(nodes_per_cluster[i])
      ...
      map(
        ...
        network.first_neighbor_ind[l]:(network.first_neighbor_ind[l + 1] - 1),
      )
    end
    map(
      k -> reduce_weights2!(
...
function MergeClusters!(clustering::MClustering, clustering_other::MClustering)
  @inbounds @fastmath @simd for i in 1:(clustering.nodes_number)
    clustering.clusters[i] = clustering_other.clusters[clustering.clusters[i]]
  end
  clustering.clusters_number = clustering_other.clusters_number
end

# --- JavaRandom ---
function Next!(java_random::JavaRandom, bits::Integer)
  java_random.seed = ((java_random.seed * UInt(0x5DEECE66D) + UInt(0xB)) & ((UInt(1) << 48) - 1))
  return Int(java_random.seed >> (48 - bits))
end

function NextInt!(java_random::JavaRandom, n::Integer)
...
function Seed!(java_random::JavaRandom, seed::Integer)
  # \xor
  java_random.seed = (UInt(seed) ⊻ UInt(0x5DEECE66D)) & ((UInt(1) << 48) - 1)
end

# --- VOSClusteringTechnique ---
function VOSct(network::NetWork, resolution::AbstractFloat)
  clustering = MClustering(network.nodes_number)
  InitSingletonClusters!(clustering)
  return VOSct(network, clustering, resolution)
end

@inline function quality_fun_sum!(k::Integer, j::Integer, vosct::VOSct, quality_fun::Base.RefValue)
  if vosct.clustering.clusters[vosct.network.neighbor[k]] == j
    quality_fun[] += vosct.network.edge_weights[k]
  end
end

@inline function clust_weights_sum!(i::Integer, cluster_weights::Base.RefValue, vosct::VOSct)
  cluster_weights[][vosct.clustering.clusters[i]] += vosct.network.node_weights[i]
end
...
@inline function Quality(vosct::VOSct)
  ...
  @inbounds @fastmath @simd for i in 1:(vosct.network.nodes_number)
    ...
    map(
      k -> quality_fun_sum!(k, j, vosct, quality_fun),
      (vosct.network.first_neighbor_ind[i]:(vosct.network.first_neighbor_ind[i + 1] - 1)),
    )
  end
  quality_fun[] += vosct.network.total_weights_self

  cluster_weights = Ref(zeros(Float32, vosct.clustering.clusters_number))
  map(i -> clust_weights_sum!(i, cluster_weights, vosct), 1:(vosct.network.nodes_number))
  map(
    i -> quality_fun[] -= cluster_weights[][i]^2 * vosct.resolution,
    1:(vosct.clustering.clusters_number),
  )
  quality_fun[] /= 2 * TotalEdgeWeights(vosct.network) + vosct.network.total_weights_self

  return quality_fun[]
end

@inline function neighbor_moving!(
  k::Int,
  vcc::Base.RefValue,
  vnn::Base.RefValue,
  vnew::Base.RefValue,
...
@inline function max_qual!(
  ...
  local quality_fun =
    edge_weights_per_cluster[][l] - vnnw[][j] * cluster_weights[l] * vosct.resolution
  if quality_fun > max_quality_fun[] || (quality_fun == max_quality_fun[] && l < best_cluster[])
    best_cluster[] = l
...
@inline function LocalMoving!(vosct::VOSct, random::JavaRandom)
  update = false
  ...
  map(
    i -> clust_node!(i, vcc, vnnw, cluster_weights, nodes_number_per_cluster),
  ...
  map(
    i -> unused!(i, nodes_number_per_cluster, unused_clusters, unused_clusters_number),
    1:(vosct.network.nodes_number),
  )
  ...
  while true
    ...
    map(
      ...
      vosct.network.first_neighbor_ind[j]:(vosct.network.first_neighbor_ind[j + 1] - 1),
    )
  ...
  map(
    i -> vosct_clust!(i, nodes_number_per_cluster, new_clusters, vosct),
    1:(vosct.network.nodes_number),
  )
  @inbounds @fastmath @simd for i in 1:(vosct.network.nodes_number)
...
function Louvain!(vosct::VOSct, random::JavaRandom)
  if vosct.network.nodes_number == 1
    return false
  end
  update = LocalMoving!(vosct, random)
  if vosct.clustering.clusters_number < vosct.network.nodes_number
    vosct_tmp = VOSct(ReducedNet(vosct.network, vosct.clustering), vosct.resolution)
...
function GenRandomPermutation!(num_ele::Integer, java_random::JavaRandom)
  permutation = collect(0:(num_ele - 1))
  map(i -> permute!(i, num_ele, java_random, permutation), 1:num_ele)
  return permutation .+ 1
end

function MatrixToNetwork(
  node1::Vector{Int},
  node2::Vector{Int},
  edge_weights::Vector{Float32},
  fun::Integer,
