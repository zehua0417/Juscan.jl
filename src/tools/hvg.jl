...
filename: hvg
author: Lihuax
date: 2025-03-05 18:13
description: Implementation of highly variable gene (HVG) selection 
in single-cell RNA sequencing data using Seurat v3 approach. This includes 
functions to compute, rank, and subset highly variable genes across batches.
=========================================#
...
function _highly_variable_genes_seurat_v3(
  adata::AnnData;
  ...
  !check_nonnegative_integers(X) &&
    @warn "flavor Seurat v3 expects raw count data, but non-integers were found"
  ...
  for b in unique(batch_info)
    ...
    norm_gene_var =
      (1 ./ ((N - 1) .* reg_std .^ 2)) .*
      ((N .* m .^ 2) .+ squared_batch_counts_sum .- 2 .* batch_counts_sum .* m)
  ...
  num_batches_high_var =
    sum(mapslices(row -> row .< n_top_genes, ranked_norm_gene_vars, dims=2), dims=1)
  ...
  median_ranked =
    mapslices(col -> mymedian(col[findall(x -> !isnan(x), col)]), ranked_norm_gene_vars, dims=1)

  sortdf = DataFrame(
    row=collect(1:length(vec(median_ranked))),
...
function subset_to_hvg!(
  ...
  if isnothing(adata.var) || (!isnothing(adata.var) && !hasproperty(adata.var, :highly_variable))
    verbose &&
      @info "no HVGs found, calculating highly variabls genes using flavor seurat v3 in-place..."
